# aigogo - Code Snippet Package Manager

You are helping the user work with aigogo, a code snippet package manager that uses Docker registries as transport. Follow these instructions exactly.
...
4. **Troubleshoot** an issue with aigogo
...
2. If not, run `aigogo init` to create one
...
4. Add source files: `aigogo add file <paths...>`
   - Use glob patterns where appropriate (e.g. `"*.py"`)
   - Respect any `.aigogoignore` file
5. Scan for dependencies: `aigogo scan`
   - Review the output and add any detected dependencies: `aigogo add dep <pkg> <version>`
   - For Python projects with `pyproject.toml`, use `aigogo add dep --from-pyproject`
6. Validate: `aigogo validate`
...
8. Build: `aigogo build`

## Workflow: Consume a Package

1. Run `aigogo add <reference>` where reference is either:
   - A registry path: `docker.io/org/package:tag`
   - A local cache reference: `package:tag`
2. Run `aigogo install` to create import symlinks
...
1. Ensure `aigogo.json` is valid: `aigogo validate`
2. Build locally: `aigogo build <name>:<tag>` or just `aigogo build` for auto-versioning
3. Test locally in another directory if needed
4. Login to registry: `aigogo login <registry>`
5. Push: `aigogo push <registry>/<name>:<tag> --from <name>:<tag>`
...
- The `--from` flag is required for `aigogo push`
