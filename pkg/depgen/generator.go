...
func (g *Generator) writePythonRequirements(path string, deps []manifest.Dependency) error {
	...
	content.WriteString("# aigogo-managed runtime dependencies\n")
	content.WriteString("# To remove: delete these entries and uninstall the packages\n\n")
...
// writePyproject generates a pyproject.toml for the package.
// Dependencies are placed in [project.optional-dependencies] under "aigogo" and
// "aigogo-dev" groups rather than [project.dependencies]. This is intentional:
// aigogo packages are distributed via registries and installed via symlinks, not
// pip. The generated pyproject.toml serves as reference/documentation. Using
// dedicated groups clearly labels deps as aigogo-managed so consumers can easily
...
func (g *Generator) writePyproject(path string, m *manifest.Manifest) error {
	...
	if m.Author != "" {
		fmt.Fprintf(&content, "authors = [{name = \"%s\"}]\n", m.Author)
	}

	fmt.Fprintf(&content, "requires-python = \"%s\"\n", m.Language.Version)

	hasRuntime := len(m.Dependencies.Runtime) > 0
	hasDev := len(m.Dependencies.Dev) > 0
	...
	if hasDev {
		content.WriteString("aigogo-dev = [\n")
...
func (g *Generator) writePackageJson(path string, m *manifest.Manifest) error {
	...
	if hasRuntime {
		...
		for i, dep := range m.Dependencies.Runtime {
			...
			fmt.Fprintf(&content, "    \"%s\": \"%s\"", dep.Package, dep.Version)
		}
		content.WriteString("\n  },\n")
	}
	...
	// aigogo metadata: tracks which deps are aigogo-managed so consumers
	// can identify and remove them. npm/yarn ignore unknown keys.
	content.WriteString("  \"aigogo\": {\n")
	if hasRuntime {
		content.WriteString("    \"managedDependencies\": [")
		for i, dep := range m.Dependencies.Runtime {
			if i > 0 {
				content.WriteString(", ")
			}
			fmt.Fprintf(&content, "\"%s\"", dep.Package)
		}
		content.WriteString("]")
		if hasDev {
			content.WriteString(",")
	...
	if hasDev {
		content.WriteString("    \"managedDevDependencies\": [")
		for i, dep := range m.Dependencies.Dev {
			if i > 0 {
				content.WriteString(", ")
			}
